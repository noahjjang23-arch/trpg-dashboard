...
### 🔗 바로 접속: <https://noahjjang23-arch.github.io/trpg-dashboard/>

**멀티플레이 설정은 이미 완료되어 있습니다.** 접속해서 `새 방 만들기` → `링크 복사` 로
일행에게 주소를 보내면 바로 같이 플레이할 수 있습니다. 아래 3장은 직접 Firebase를
새로 붙일 때(포크했거나 다른 프로젝트를 쓸 때)를 위한 설명입니다.
...
> 이 저장소는 이미 Firebase 프로젝트 **TRPG (`trpg-b4f13`)** 에 연결되어 있습니다.
> 아래 내용은 저장소를 포크했거나 **본인 소유의 다른 Firebase 프로젝트로 바꾸고 싶을 때** 따라 하세요.

Firebase 설정이 없으면 각자의 브라우저에 따로 저장되는 "로컬 단독 플레이"가 됩니다.
...
js/
  ...
  firebase_config.js       Firebase 웹 앱 설정 (공개되어도 되는 값)
```
